...
namespace VueJSMVCDotNet
{
    ...
    {
        public static IEnumerable<ParameterInfo> StripMethodParameters(MethodInfo method)
            => StripMethodParameters(method.GetParameters());

        public static IEnumerable<ParameterInfo> StripMethodParameters(ParameterInfo[] parameters)
            => parameters
                .Where((p, index) =>
                {
        ...
        public async Task<T?> InvokeAsync<T>(object? obj, IRequestData requestData, object?[]? pars = null, AddItem? addItem = null, IHeaderDictionary? responseHeaders = null)
        {
