...
{
    internal class ModelInstanceMethodsGenerator : IJSGenerator
    {
        public void GeneratorJS(ref WrappedStringBuilder builder, Type modelType, string urlBase)
        {
            ...
            foreach (MethodInfo mi in Utility.GetModelMethods(modelType, false))
            {
...
for(let x=0;x<{0}.length;x++){{
    function_data.{0}.push({{id:{0}[x].id}});
                ...
                builder.AppendLine(string.Format(@"             let model = this;
